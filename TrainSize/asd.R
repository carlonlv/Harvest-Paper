...
for (i in train_size) {
  ...
  bg_param_setting <- expand.grid(window_size = window_size, train_policy = train_policy, train_size = train_size, stringsAsFactors = FALSE)
  bg_param_setting <- cbind(bg_param_setting, data.frame(name = "ARIMA", granularity = granularity, train_size = i, update_freq = 3, react_speed = "1,2", extrap_step = 1, stringsAsFactors = FALSE))
  ...
  bg_param_setting <- expand.grid(window_size = window_size, train_policy = train_policy, train_size = train_size, cluster_type = c("fixed", "quantile"), stringsAsFactors = FALSE)
  bg_param_setting <- cbind(bg_param_setting, data.frame(name = "MARKOV", cluster_type = "quantile", granularity = granularity, train_size = i, update_freq = 3, react_speed = "1,2", extrap_step = 1, stringsAsFactors = FALSE))
  ...
  bg_param_setting <- expand.grid(window_size = window_size, train_policy = train_policy, train_size = train_size, stringsAsFactors = FALSE)
  bg_param_setting <- cbind(bg_param_setting, data.frame(name = "VAR", p = 1, granularity = granularity, train_size = i, update_freq = 3, react_speed = "1,2", extrap_step = 1, stringsAsFactors = FALSE))
  bg_param_setting$window_size_for_xreg <- bg_param_setting$window_size
  bg_param_setting$window_type_for_xreg <- "avg"
