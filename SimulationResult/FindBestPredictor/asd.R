...
bg_param_setting <- data.frame(class = "ARIMA", name = "AR1X(Avg)", window_size = window_size, granularity = 0, train_policy = "fixed", train_size = 2000, update_freq = 3, react_speed = "1,2", extrap_step = 1, stringsAsFactors = FALSE)
...
bg_param_setting <- data.frame(class = "MARKOV", name = "MarkovX(Avg)", window_size = window_size, granularity = 0, train_policy = "fixed", train_size = 2000, update_freq = 3, react_speed = "1,2", extrap_step = 1, stringsAsFactors = FALSE)
...
additional_setting <- list("cut_off_prob" = cut_off_prob, "include_response_window_size" = TRUE, "window_size_for_reg" = 12, "window_type_for_reg" = c("avg", "max"))
...
d <- run_sim(bg_param_setting, additional_setting, microsoft_max_10000, list(microsoft_avg_10000, microsoft_max_10000), cores = parallel::detectCores(), write_type = c("charwise", "paramwise"), plot_type = "none", result_loc = "~/SimulationResult/FindBestPredictor/AR1/")

additional_setting <- list("cut_off_prob" = cut_off_prob, "include_response_window_size" = TRUE, "window_size_for_reg" = 12, "window_type_for_reg" = "max")
bg_param_setting <- data.frame(class = "MARKOV", name = "MarkovX(Max12)", "include_response_window_size" = TRUE, window_size = window_size, granularity = 0, train_policy = "fixed", train_size = 2000, update_freq = 3, react_speed = "1,2", extrap_step = 1, stringsAsFactors = FALSE)
