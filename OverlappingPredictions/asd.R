...
for (i in 1:nrow(ddd)) {
  ...
  pre_compute_models_foreground(paste0("FPR_AR1_", ddd[i, "window_size"], "_", ddd[i, "granularity"], ".rda"), param_setting_sim = bg_param_setting, additional_param_sim = list(), foreground_x = microsoft_max_10000, foreground_xreg = NULL, sim_length = 1000, bins = ddd[i, "window_size"], cores = parallel::detectCores())
}
